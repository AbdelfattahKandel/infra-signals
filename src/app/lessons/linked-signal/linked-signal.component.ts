 ...
 */
protected readonly fullName = linkedSignal(() => `${this.firstName()} ${this.lastName()}`);
protected readonly fullNameReadOnly = computed(() => `${this.firstName()} ${this.lastName()}`);
...
protected readonly updateFullName = () => {
  this.fullName.update((fullName) => fullName === 'Abdo Atef' ? 'Tarek Ahmed' : 'Abdo Atef');
  console.log(this.fullName());
